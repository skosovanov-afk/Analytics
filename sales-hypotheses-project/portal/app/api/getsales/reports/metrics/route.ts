  ...
  try {
    ...
    if (kind !== "linkedin") {
      return NextResponse.json({
        ok: true,
        disabled: true,
        reason: "GetSales email metrics are disabled (LinkedIn-only mode).",
        kind,
        since: sinceYmd,
        until: untilYmd,
        metrics: { total: {} }
    ...
    // Read from transactional GetSales events to match UI counts.
    const rows = await listGetsalesEventsPaged(supabaseAdmin, sinceYmd, untilYmd);
    console.log("[GetSales Metrics] rows", {
      since: sinceYmd,
      until: untilYmd,
      count: rows.length
    });
    ...
    const totals: Record<string, number> = {
      ...
      linkedin_opened_count: 0,
      linkedin_inbox_count: 0
    };

    const connSentLeads = new Set<string>();
    const connAcceptedLeads = new Set<string>();
    const msgSentKeys = new Set<string>();
    const msgOpenedKeys = new Set<string>();
    const msgRepliedKeys = new Set<string>();
    ...
    console.log("[GetSales Reports] metrics query", {
      sinceYmd,
      untilYmd,
    ...
    for (const r of (rows || [])) {
      ...
      if (kind === "linkedin_connection_request_sent") {
        if (r?.lead_uuid) connSentLeads.add(String(r.lead_uuid));
      } else if (kind === "linkedin_connection_request_accepted") {
        if (r?.lead_uuid) connAcceptedLeads.add(String(r.lead_uuid));
      } else if (kind === "linkedin_message_sent") {
        if (isInmail(r?.payload)) continue;
        const key = msgKey(r?.payload);
        if (key) msgSentKeys.add(key);
      } else if (kind === "linkedin_message_opened") {
        if (isInmail(r?.payload)) continue;
        const key = msgKey(r?.payload);
        if (key) msgOpenedKeys.add(key);
      } else if (kind === "linkedin_message_replied") {
        if (isInmail(r?.payload)) continue;
        const key = msgKey(r?.payload);
        if (key) msgRepliedKeys.add(key);
      }
    }

    totals.linkedin_connection_request_sent_count = connSentLeads.size;
    totals.linkedin_connection_request_accepted_count = connAcceptedLeads.size;
    totals.linkedin_sent_count = msgSentKeys.size;
    totals.linkedin_opened_count = msgOpenedKeys.size;
    totals.linkedin_inbox_count = msgRepliedKeys.size;

    console.log("[GetSales Reports] metrics totals", {
      connections_sent: totals.linkedin_connection_request_sent_count,
      connections_accepted: totals.linkedin_connection_request_accepted_count,
      messages_sent: totals.linkedin_sent_count,
      messages_opened: totals.linkedin_opened_count,
      messages_replied: totals.linkedin_inbox_count
    });

    console.log("[GetSales Metrics] totals", {
      connections_sent: totals.linkedin_connection_request_sent_count,
      connections_accepted: totals.linkedin_connection_request_accepted_count,
      messages_sent: totals.linkedin_sent_count,
      messages_opened: totals.linkedin_opened_count,
