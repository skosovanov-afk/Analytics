          ...
          <>
            <svg viewBox={`0 0 ${w} ${h}`} width="100%" height={h} style={{ display: "block" }}>
              ...
              {weeks.map((wk, i) => {
                ...
                const x = xFor(i);
                return (
                  <text key={wk} x={x} y={h - 8} textAnchor="middle" fontSize="11" fill="var(--chartAxis)">
                    {wk.slice(5)}
                  </text>
                );
              })}
            ...
            <div className="chartLegend">
              {series.map((s, i) => (
                <span key={s.key} className="chartLegendItem">
                  <span className="chartLegendSwatch" style={{ width: 10, height: 10, background: COLORS[i % COLORS.length] }} />
                  <span>{s.label}</span>
                </span>
              ))}
...
export default function AdvancedDashboardPage() {
  ...
  async function load() {
    ...
    if (cRes.error) {
      setCheckins([]);
      if (isMissingTableError(cRes.error.message)) {
        setStatus("Check-ins table is missing in Supabase. Dashboard loaded without retrospective check-ins.");
      } else {
        setStatus(`checkins error: ${cRes.error.message}`);
      }
    } else {
